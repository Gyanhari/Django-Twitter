...
def index(request):
    ...
    for usernames in user_following_list:
        feed_lists = Post.objects.filter(user=usernames, approved=True)  # Filter approved posts
    ...
    return render(request, 'index.html', {'user_profile': user_profile, 'user_email': user_email, 'posts': feed_list,
...
def upload(request):
    if request.method == 'POST':
        ...
        if not image or not caption:  # Check if any required field is empty
            messages.error(request, "Please fill in all the required fields.")
            return redirect('/')
        else:
            new_post = Post.objects.create(user=user, image=image, caption=caption, first_name=first_name, last_name=last_name)
            new_post.save()
...
def admin_approval(request):
    ...
    return render(request, 'admin_approval.html', {'pending_posts': pending_posts, 'approved_posts': approved_posts})
